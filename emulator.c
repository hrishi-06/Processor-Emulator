#include <stdio.h>
#include <stdlib.h>

// checks if a valid register is accessed (by testing if op < 8)
// exits if op >= 8 as it makes the instruction illegal
void check_index(unsigned char op)
{
	if(op < 8) {
		return;
	}
	else {
		printf("Illegal instruction\n");
		exit(-1);
	}
}

unsigned char memory[256] = {
	
};

unsigned char registers[8] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

int main ()
{
	int instruction_pointer = 0;
	
	while(1) {
		unsigned char op0 = memory[instruction_pointer+0];
		unsigned char op1 = memory[instruction_pointer+1];
		unsigned char op2 = memory[instruction_pointer+2];
		
		instruction_pointer += 3;

		int index1 = op1;
		int index2 = op2;

		switch(op0) {
			case 0x00:
				check_index(op1);
				registers[index1] = op2;
				break;
			case 0x01:
				check_index(op1);
				registers[index1] += op2;
				break;
			case 0x02:
				check_index(op1);
				registers[index1] -= op2;
				break;
			case 0x03:
				check_index(op1);
				check_index(op2);
				registers[index1] += registers[index2];
				break;
			case 0x04:
				check_index(op1);
				check_index(op2);
				registers[index1] -= registers[index2];
				break;
			case 0x05:
				check_index(op1);
				printf("%c", registers[index1]);
				break;
			case 0x06:
				check_index(op1);
				if(registers[index1] != 0){
					instruction_pointer = index2;
				}
				break;
			case 0x07:
				check_index(op1);
				if(registers[index1] == 0){
					instruction_pointer = index2;
				}
				break;
			case 0x08:
				check_index(op1);
				check_index(op2);
				registers[index1] = memory[registers[index2]];
				break;
			case 0x09:
				check_index(op1);
				check_index(op2);
				registers[index2] = memory[index1];
				break;
			case 0xff:
				return 0;
			default:
				printf("Illegal instruction\n");
				return -1;
		}
	}

	return 0;
}
